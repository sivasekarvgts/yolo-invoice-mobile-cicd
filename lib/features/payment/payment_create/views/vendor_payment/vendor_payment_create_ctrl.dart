...
part 'vendor_payment_create_ctrl.g.dart';

@riverpod
class VendorPaymentCreateCtrl extends _$VendorPaymentCreateCtrl
    with PaymentMasterServices {
  @override
  ...
  Future _updateSalesPayment() async {
    ...
    try {
      final res = await ref
          ...
          .updatePayment(billId, getPaymentReq, isVendor);
      if (res == null) return;
  ...
  Future _createSalesPayment() async {
    ...
    try {
      final res = await ref
          .read(paymentCreateRepoProvider)
          .createPayment(getPaymentReq, isVendor);
      if (res == null) return;
      toastMsg(res['message'].toString(), false);
      navigationService.pop();
  ...
  PaymentCreateRequestModel get getPaymentReq {
    ...
    final paymentRequest = PaymentCreateRequestModel(
        vendorId: clientId,
        account: selectedAccountItem?.id,

...
part 'sales_order_list_controller.g.dart';

@riverpod
class SalesOrderListController extends _$SalesOrderListController
    with HideFloating {
  @override
  ...
  OrderListModel? salesOrderData;
  List<OrderListItem> salesOrderList = [];
  ...
  void onRefreshEvent() async {
    eventBusService.eventBus.on<PageRefreshEvent>().listen((event) async {
      if (event.pageNames.contains(Routes.salesOrderList))
        await fetchData(refresh: event.isRefresh);
    });
  }

  // TODO check refresh
  // void onSalesOrderRefreshEvent() async {
  //   eventBusService.eventBus.on<SalesOrderRefreshEvent>().listen((event) async {
  ...
  Future fetchData({bool refresh = false, bool isDateFilter = false}) async {
    ...
    int numPages = (salesOrderData?.numPages ?? 1);
    int currentPage = (salesOrderData?.currentPage ?? 0);
    ...
    try {
      ...
      salesOrderList.addAll(salesOrderData?.orderList ?? []);
    } catch (e, s) {
  ...
  void openBillPreview(int i) {
    final sales = salesOrderList[i];
    navigationService.pushNamed(Routes.billPreview,
        arguments: BillPreviewArgs(orderId: sales.id,isSalesOrder: true));
  }

  void createSalesOrder() async {
    final res =
        await navigationService.pushNamed(Routes.customer, arguments: true);
    if (res == null) return;
    navigationService.pushNamed(Routes.salesOrder,
        arguments: SalesParamsRequestModel(clientId: res,billType: BillType.salesOrder,));
  }
}
